...
export const proffAppointmentDisplayByFilter = async (req, res) => {
  try {
    ...
    if (professionalIds.length === 0) {
      console.log('No matching appointments found; falling back to professional directory lookup');
      const fallbackProfessionals = await Professional.find({ role })
        .select('pName pID qualification experience profilePicture');

      console.log('Fallback professionals count:', fallbackProfessionals.length);

      result = fallbackProfessionals.map((prof) => ({
        pID: prof.pID,
        pName: prof.pName || 'Unknown',
        qualification: prof.qualification || 'Not specified',
        experience: prof.experience || 'Not specified',
        profilePicture: prof.profilePicture || 'https://via.placeholder.com/300x200',
        chargePerAppointment: prof.chargePerAppointment || 0,
        specialNotes: 'Contact to check availability',
        appointmentDate: 'Not scheduled',
        availableTime: 'Flexible',
      }));
    } else {
      const professionals = await Professional.find({ pID: { $in: professionalIds } })
        .select('pName pID qualification experience profilePicture chargePerAppointment');
      ...
      result = appointments.map((appointment) => {
        ...
        return {
          ...
          chargePerAppointment: appointment.chargePerAppointment || prof?.chargePerAppointment || 0,
