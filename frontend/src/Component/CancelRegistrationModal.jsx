  ...
  return (
    <div className="fixed inset-0 bg-gray-800/30 backdrop-blur-lg flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
        <h2 className="text-2xl font-bold text-[#FF5C7A] mb-4">Cancel Registration</h2>
        ...
        <p className="text-gray-600 mb-4">
          Refund Amount: <span className="font-medium text-[#FF7E67]">${calculateRefundAmount().toFixed(2)}</span> (50% of total if cancelled before event date)
        </p>
        <p className="text-gray-600 mb-4">Please provide a reason for cancellation:</p>
        <textarea
          value={reason}
          onChange={(e) => setReason(e.target.value)}
          className="w-full p-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-[#FF7E67] bg-gray-50"
          rows="4"
