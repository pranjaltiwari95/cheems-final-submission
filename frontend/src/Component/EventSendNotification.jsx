...
const SendNotification = ({ attendees, eventId }) => {
  ...
  const handleSendNotification = async () => {
    if (!message.trim()) {
      ...
      setTimeout(() => setErrorMessage(null), 5003);
      return;
    }
    ...
    <div className="bg-white/80 rounded-2xl shadow-lg p-8 border border-amber-100 mt-8">
      ...
      <div className="relative mb-4">
        <textarea
          ...
          className="w-full h-32 p-4 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#FF7E67] focus:border-transparent text-base resize-none"
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-gradient-to-r from-[#FF7E67] to-[#FF5C7A] text-white px-6 py-3 rounded-xl hover:bg-[#2D1B3D] transition-colors duration-300 transform hover:scale-105 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
        >
          {loading ? (
        ...
        {sentNotifications.length > 0 ? (
          <div className="space-y-3">
            {sentNotifications.map((notification) => (
              <div
                key={notification.id}
                className="bg-gray-50 p-4 rounded-xl border-l-4 border-[#FF7E67] hover:shadow-md transition duration-300"
              >
