...
const StoreAdminOrders = ({ token }) => {
  ...
  const getStatusColor = (status) => {
    switch(status) {
      ...
      default:
        return 'bg-[#FF7E67]/10 text-[#FF7E67] border-[#FF7E67]/20';
    }
  };
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="flex flex-col items-center">
          <div className="w-16 h-16 border-4 border-[#FF7E67] border-t-transparent rounded-full animate-spin"></div>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="bg-white p-8 rounded-xl shadow-lg max-w-md w-full">
          ...
          <button 
            onClick={fetchOrders}
            className="w-full px-4 py-2 bg-gradient-to-r from-[#2D1B3D] to-[#FF7E67] text-white rounded-lg hover:opacity-90 transition flex items-center justify-center"
          >
    ...
    <div className="p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-extrabold text-gray-800 flex items-center">
          <Package className="mr-3 text-[#FF7E67]" /> Order Management
        </h2>
        <button
          onClick={fetchOrders}
          className="px-4 py-2 bg-white text-[#FF7E67] rounded-lg border border-[#FF7E67] hover:bg-[#FF7E67]/5 transition flex items-center"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div className="bg-white rounded-xl shadow-md p-6 flex items-center hover:shadow-lg transition-shadow">
          <div className="rounded-full bg-[#FF7E67]/20 p-3 mr-4">
            <IndianRupee className="h-6 w-6 text-[#FF7E67]" />
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#FF7E67] focus:border-transparent"
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center">
              <Filter className="w-4 h-4 mr-1 text-[#FF7E67]" /> Filter by Product:
            </label>
            <select
              value={selectedCategory}
              onChange={(e) => setSelectedCategory(e.target.value)}
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#FF7E67] focus:border-transparent"
            >
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center">
              <Calendar className="w-4 h-4 mr-1 text-[#FF7E67]" /> Filter by Date:
            </label>
            <select
              value={dateFilter}
                ...
                {filteredOrders.map((order) => (
                  <React.Fragment key={order._id}>
                    ...
                    >
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-[#FF7E67]/20 flex items-center justify-center">
                        ...
                        <div className="max-w-xs">
                          {order.products && order.products.slice(0, 2).map((product, index) => (
                            <div key={index} className="flex items-center mb-1 gap-2">
                              <span className="inline-block w-2 h-2 rounded-full bg-[#FF7E67]"></span>
                          ...
                          {order.products && order.products.length > 2 && (
                            <div className="text-sm text-[#FF7E67]">+ {order.products.length - 2} more items</div>
                          )}
                        </div>
                      </td>
                      <td className="py-4 px-4">
                      ...
                      <td className="py-4 px-4" onClick={(e) => e.stopPropagation()}>
                        <div className="relative">
                          <select
                            ...
                            className={`px-3 py-2 rounded-lg text-sm font-medium border ${getStatusColor(order.status)} focus:outline-none focus:ring-2 focus:ring-[#FF7E67] appearance-none bg-white pl-8 pr-10 ${statusUpdateLoading ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer'}`}
                      ...
                      <tr>
                        <td colSpan="6" className="py-4 px-4 bg-gray-50 border-b border-gray-200">
                          <div className="p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              <div>
                                <h4 className="text-lg font-semibold text-gray-800 mb-3 flex items-center">
                                  <User className="w-5 h-5 mr-2 text-[#FF7E67]" /> Customer Details
                                </h4>
                              ...
                              <div>
                                <h4 className="text-lg font-semibold text-gray-800 mb-3 flex items-center">
                                  <Package className="w-5 h-5 mr-2 text-[#FF7E67]" /> Order Summary
                                </h4>
                                <div className="space-y-2 text-sm">
                            ...
                            <div className="mt-6">
                              <h4 className="text-lg font-semibold text-gray-800 mb-3 flex items-center">
                                <ShoppingBag className="w-5 h-5 mr-2 text-[#FF7E67]" /> Order Items
                              </h4>
                              <div className="overflow-x-auto">
