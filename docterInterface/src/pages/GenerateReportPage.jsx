...
const Notification = ({ message, type, onClose }) => {
  useEffect(() => {
    const timer = setTimeout(() => {
      onClose();
    }, 5003); // Auto-close after 5 seconds
    ...
    const validateTokenAndFetchData = async () => {
      ...
      try {
        ...
        const [incomeResponse, petsUsersResponse] = await Promise.all([
          axios.get('http://localhost:5003/api/appointments/income-report', {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get('http://localhost:5003/api/appointments/pets-users-report', {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
