...
const PlaceOrder = () => {
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
        ...
        const response = await axios.post(`${BACKEND_URL}/api/order/cod`, orderData, {
            headers: { 
      ...
      <form onSubmit={handlePlaceOrder} className='max-w-4xl mx-auto bg-white rounded-xl shadow-md p-8 flex flex-col gap-8 '>
        ...
        <div className='flex flex-col gap-4'>
          <div className='flex gap-3'>
            <input 
              className='border border-amber-200 bg-amber-50 rounded-lg py-2 px-4 w-full focus:ring-2 focus:ring-[#FF7E67] focus:border-[#FF7E67] text-amber-900' 
              type='text' 
              name='firstName'
        ...
        <div>
          ...
          <div className='flex gap-4 flex-col lg:flex-row'>
            <div className='flex items-center gap-3 border-2 border-[#FF7E67] p-3 px-5 rounded-lg bg-amber-100'>
              <span className='min-w-3.5 h-3.5 border rounded-full bg-green-400 border-green-400'></span>
              <span className='text-amber-900 font-medium mx-4'>Cash on Delivery</span>
            </div>
          </div>
          <p className='text-sm text-amber-700 mt-3'>
            Online payments are currently disabled. Please pay at the time of delivery.
          </p>
          <div className='w-full text-end mt-8'>
            <button type="submit" className='bg-[#FF7E67] hover:bg-[#FF6C85] text-white font-bold rounded-full shadow-md text-base px-16 py-3 transition-all duration-200'>PLACE ORDER</button>
          </div>
        </div>
      </form>
    </div>
