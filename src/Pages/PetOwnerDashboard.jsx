...
const PetOwnerDashboard = () => {
  ...
  const handleDownloadReport = (pet) => {
    const doc = new jsPDF();
    // Header with color
    doc.setFillColor(208, 136, 96); // #FF7E67
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#FFE6DD] to-[#FFD8CC] pt-24 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-[#2D1B3D]">Loading your pet listings...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#FFE6DD] to-[#FFD8CC] pt-24 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-[#2D1B3D]">My Pet Listings</h1>
            <p className="text-gray-600 mt-2">Manage your pets listed for adoption</p>
          </div>
          <button
            onClick={handleAddNewPet}
            className="bg-[#FF5C7A] text-white px-6 py-3 rounded-xl hover:bg-[#FF7E67] transition-colors flex items-center gap-2"
          >
            <PlusCircle size={20} />
            Add New Pet
          </button>
                ...
                <div className="flex min-h-[250px]">
                  ...
                  <div className="flex-1 p-6">
                    ...
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        <h3 className="text-2xl font-semibold text-[#2D1B3D] mb-1">{pet.petName}</h3>
                        ...
                        {!isPetAdopted(pet._id) && (
                          <>
                            <button
                              ...
                              className="p-2 text-[#FF5C7A] hover:bg-[#FF5C7A]/10 rounded-lg transition-colors"
                            >
                              <Edit size={20} />
                            </button>
                            <button
                    ...
                    <div className="grid grid-cols-2 gap-x-6">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-3">
                          <Clock className="w-5 h-5 text-[#FF5C7A]" />
                          <div>
                            <span className="font-medium">Age:</span> {pet.petAge}
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <Heart className="w-5 h-5 text-[#FF5C7A]" />
                          <div>
                            <span className="font-medium">Gender:</span> {pet.petGender}
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <Shield className="w-5 h-5 text-[#FF5C7A]" />
                          <div>
                        ...
                        <div className="flex items-center gap-3">
                          <Syringe className="w-5 h-5 text-[#FF5C7A]" />
                          <div>
                            <span className="font-medium">Vaccinated:</span> {pet.vaccinated ? 'Yes' : 'No'}
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <Scissors className="w-5 h-5 text-[#FF5C7A]" />
                          <div>
                      ...
                      <div>
                        <h4 className="font-medium text-[#2D1B3D] mb-4">Owner Contact</h4>
                        <div className="space-y-4">
                          <div className="flex items-center gap-3">
                            <Mail className="w-5 h-5 text-[#FF5C7A]" />
                            <p>{pet.email}</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <Phone className="w-5 h-5 text-[#FF5C7A]" />
                            <p>{pet.phone}</p>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="mt-6 border-t border-gray-100 pt-6">
                      <div className="space-y-4">
                        <div>
                          <h4 className="font-medium text-[#2D1B3D] mb-2">Pet Description</h4>
                          <p className="text-gray-600">
                            {pet.petDescription || 'No description available'}
                          </p>
                        </div>
                        <div>
                          <h4 className="font-medium text-[#2D1B3D] mb-2">Reason for Adoption</h4>
                          <p className="text-gray-600">
        ...
        {pets.length === 0 && !error && (
          <div className="text-center py-12">
            <Heart className="mx-auto text-[#FF5C7A]" size={48} />
            <h3 className="text-xl font-semibold text-[#2D1B3D] mt-4">No Pets Listed Yet</h3>
            <p className="text-gray-600 mt-2">Start by adding your first pet for adoption</p>
            <button
              onClick={handleAddNewPet}
              className="mt-4 bg-[#FF5C7A] text-white px-6 py-3 rounded-xl hover:bg-[#FF7E67] transition-colors"
            >
              Add Your First Pet
            </button>
          </div>
        ...
        {showDeleteConfirm.show && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-8 rounded-xl max-w-md w-full mx-4">
              <h3 className="text-xl font-semibold text-[#2D1B3D] mb-4">Confirm Deletion</h3>
