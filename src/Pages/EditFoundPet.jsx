...
const EditFoundPet = () => {
  ...
  const fetchPetDetails = async () => {
    try {
      ...
      const response = await axios.get(`https://cheems-final-submission-1.onrender.com/api/lost-and-found/found/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`https://cheems-final-submission-1.onrender.com/api/lost-and-found/found/${id}`, formDataToSend, {
        headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('petOwnerToken') || localStorage.getItem('adminToken');
      await axios.delete(`https://cheems-final-submission-1.onrender.com/api/lost-and-found/found/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FFEADF] to-[#FFEADF] py-8 mt-20">
      <div className="max-w-2xl mx-auto px-4">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-[#2D1B3D] mb-4">Edit Found Pet Report</h1>
          <p className="text-lg text-gray-600">Update the details of the found pet</p>
        </div>

        <div className="bg-white rounded-xl shadow-lg p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-[#2D1B3D]">Edit Found Pet Report</h2>
            <button
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FF7E67]"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  required
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-[#FF7E67] focus:border-[#FF7E67]"
                />
              </div>
            ...
            <div className="flex justify-end gap-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="bg-[#FF7E67] text-white px-6 py-3 rounded-xl hover:bg-[#2D1B3D] transition-colors disabled:opacity-50"
              >
