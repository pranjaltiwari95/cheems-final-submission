...
import { FaUser, FaPhone, FaEnvelope, FaPaw, FaCalendar, FaClock, FaRupeeSign } from "react-icons/fa";
...
const AppointmentForm = ({ professional, onClose, appointmentType}) => {
  ...
  const renderInputField = (name, label, type, icon, options = null, readOnly = false) => {
    ...
    const inputClass = `w-full px-4 py-3 border rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#FF7E67] focus:border-transparent transition duration-200 ${
      ...
      return (
        <div className="mb-6">
          <label className="block font-medium mb-2 flex items-center text-gray-700">
            {IconComponent && <IconComponent className="mr-2 text-[#FF7E67]" size={18} />}
            {label}
          </label>
  ...
  return (
    <div className="max-w-xl mx-auto p-8 shadow-xl rounded-xl bg-white mt-8 border border-gray-100">
      <div className="text-center mb-8">
        <div className="inline-block p-3 rounded-full bg-[#FF7E67] bg-opacity-10 mb-4">
          <FaCalendar className="text-[#FF7E67]" size={24} />
        </div>
        <h2 className="text-2xl font-bold text-[#FF7E67]">
          Book an Appointment
        </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>{renderInputField("appointmentFee", "Appointment Fee (₹)", "number", FaRupeeSign, null, true)}</div>
        ...
        <div className="mt-8 flex justify-between">
          ...
          <button
            type="submit"
            className={`px-6 py-3 bg-[#FF7E67] text-white rounded-lg hover:bg-[#FF6C85] transition-all duration-300 shadow-sm flex items-center ${
