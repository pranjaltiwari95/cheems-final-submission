...
export default function PetTrainerAvailability() {
  ...
  useEffect(() => {
    const fetchAvailabilities = async () => {
      try {
        ...
        const dataArray = Array.isArray(trainerData) ? trainerData : [];
  ...
  if (error) {
    return (
      <div className="text-center p-6 text-red-500 bg-red-50 rounded-lg mx-auto max-w-2xl animate-fade-in">
        <p className="font-semibold">Error loading pet trainer availabilities</p>
        <p>{error}</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto p-6 bg-gradient-to-br from-[#FFEADF] to-[#FFE1D6] min-h-screen relative">
      ...
      <h2 className="text-2xl font-semibold mb-6 flex items-center text-amber-950 animate-fade-in">
        ...
        Pet Trainer Availability Requests
      </h2>
              ...
              {filteredAvailabilities.length === 0 ? (
                <tr className="animate-fade-in">
                  <td colSpan="8" className="text-center py-6 text-amber-950">
                    No pet trainer availability requests found
                  </td>
                </tr>
              ) : (
