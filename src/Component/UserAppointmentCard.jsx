...
export default function UserAppointmentCard() {
  ...
  const theme = {
    primary: "bg-gradient-to-r from-[#FF7E67] to-[#FF5C7A]",
    secondary: "bg-[#2D1B3D]",
    textPrimary: "text-white",
    textSecondary: "text-gray-900",
    accent: "bg-[#FFEADF]",
    border: "border-[#FFE1D6]",
  };
  ...
  return (
    <div className="relative max-w-7xl mx-auto px-4 py-8 pt-24 bg-gradient-to-br from-[#FFEADF] to-[#FFE1D6]">
      ...
      <div className={isViewingDetails || isBooking ? "filter blur-lg" : ""}>
        <div className={`${theme.accent} rounded-2xl p-6 md:p-10 mb-8 shadow-lg transform hover:shadow-xl transition-all duration-300`}>
          <h1 className="text-4xl font-bold mb-4 text-center text-gray-900">
            Cheems <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FF7E67] to-[#FF5C7A]">Professionals</span>
          </h1>
          ...
          <div className="relative max-w-md mx-auto">
            ...
            <input
              ...
              className="w-full py-3 pl-10 pr-4 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#FF7E67] focus:border-transparent bg-white/80 shadow-sm hover:shadow-md transition-all duration-300"
            />
          </div>
        ...
        <div className="block">
          <div className="flex justify-between items-center mb-8 relative">
            <h2 className="text-3xl font-bold text-gray-900">
              Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FF7E67] to-[#FF5C7A]">{activeSection.charAt(0).toUpperCase() + activeSection.slice(1)}s</span>
            </h2>
            <span className="bg-[#FFEADF] text-[#FF5C7A] px-4 py-1 rounded-xl text-sm font-medium shadow-sm">
...
function ProfessionalCard({ id, imgSrc, name, title, description, fee, onViewDetails, theme, availableTime, appointmentDate }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl shadow-lg bg-white/80 flex flex-col transition-all duration-300 hover:shadow-xl hover:scale-105 border border-[#FFE1D6] group">
      <div className="relative pt-[56.25%]">
        <img src={imgSrc} alt={name} className="absolute top-0 left-0 w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
        <div className="absolute top-4 left-4 bg-white/90 px-2 py-1 rounded text-xs font-semibold text-[#FF5C7A]">{id}</div>
      </div>

      <div className="p-6 flex-grow flex flex-col">
        <div className="flex justify-between items-start mb-3">
          <div>
            <h3 className="text-xl font-semibold text-[#FF5C7A]">{name}</h3>
...
function ProfessionalDetails({ professional, theme, onClose, onBook }) {
  return (
    <div className="p-8 shadow-xl rounded-2xl bg-white/95 border border-[#FFE1D6] transform transition-all duration-300">
