...
export default function AppointmentMngHeroPart() {
  ...
  useEffect(() => {
    ...
    }, 5003); // Change image every 5 seconds
    return () => clearInterval(interval);
  }, [images.length]);

  return (
    <div
      className="min-h-screen py-24 sm:py-32 animate-fade-in"
      style={{
        backgroundImage: `linear-gradient(rgba(255, 202, 40, 0.7), rgba(255, 213, 79, 0.7)), url(${images[currentImageIndex]})`,
    ...
    >
      <div className="mx-auto max-w-2xl px-6 lg:max-w-7xl lg:px-8">
        {/* Hero Text Overlay */}
        <div className="text-center py-20 animate-slide-up" style={{ animationDelay: '0.5s' }}>
          <h2 className="text-base/7 font-semibold text-amber-50">
            Efficiently Manage Appointments
          </h2>
          <p className="mx-auto mt-2 max-w-lg text-4xl font-semibold tracking-tight text-amber-100 sm:text-5xl">
            ...
            {images.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  currentImageIndex === index ? 'bg-amber-500 scale-125' : 'bg-amber-300 hover:bg-amber-400'
                }`}
        ...
        <div className="mt-16 grid gap-6 sm:mt-20 lg:grid-cols-3 lg:grid-rows-2 animate-slide-up" style={{ animationDelay: '1s' }}>
